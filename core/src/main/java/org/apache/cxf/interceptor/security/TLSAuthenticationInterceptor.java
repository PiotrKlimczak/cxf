...
public class TLSAuthenticationInterceptor extends AbstractPhaseInterceptor<Message> {
    ...
    protected String getUserName(X509Certificate certificate) {
        ...
        for (Rdn rdn : ldapDn.getRdns()) {
            if (getSubjectDnKey().equalsIgnoreCase(rdn.getType())) {
                return (String) rdn.getValue();
            }
        }

        throw new SecurityException("No " + getSubjectDnKey() + " key found in certificate DN: " + dn);
    }

    public String getSubjectDnKey() {
        return subjectDnKey;
    }

    public void setSubjectDnKey(String subjectDnKey) {
        this.subjectDnKey = subjectDnKey;
    }
    ...
    private class DefaultTLSSecuritySubjectProvider extends TLSSecuritySubjectProvider {
        ...
        @Override
        Subject getSubject(String userName, X509Certificate certificate) throws SecurityException {
